	...
	float* __restrict__ dL_dsemantic_feature) 
{
	...
	__shared__ float collected_semantic_feature[NUM_SEMANTIC_CHANNELS * BLOCK_SIZE];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < NUM_SEMANTIC_CHANNELS; i++)
				collected_semantic_feature[i * BLOCK_SIZE + block.thread_rank()] = semantic_feature[coll_id * NUM_SEMANTIC_CHANNELS + i];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < NUM_SEMANTIC_CHANNELS; ch++) 
			{
				...
				dL_dalpha += (c - accum_rec[ch]) * dL_dfeaturechannel;
